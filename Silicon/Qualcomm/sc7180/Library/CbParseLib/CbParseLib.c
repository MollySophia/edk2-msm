...
{
  DEBUG ((DEBUG_INFO, "%a Entry, Header = 0x%llx\n", __func__, (UINTN)Header));
  UINT16  CheckSum;
  ...
  DEBUG ((DEBUG_INFO, "Valid CbTable found at 0x%llx\n", (UINTN)Header));
  ...
  )
{
  ...
  //
  // Find simplified coreboot table at memory offset 0 ~ 4KB
  //
  for (Idx = 0; Idx < 4096; Idx += 16) {
    Ptr = (struct cb_header *)(Header + Idx);
    if (Ptr->signature == CB_HEADER_SIGNATURE) {
      break;
    }
  }
  ...
  //
  // CbTablePtr = NULL;
  // TmpPtr     = (UINT8 *)Ptr + Ptr->header_bytes;
  ...
  )
{
  ...
  Rec = (CB_MEMORY *)FindCbTag (CB_TAG_MEMORY);
  if (Rec == NULL) {
    DEBUG ((DEBUG_ERROR, "Failed to find CB_TAG_MEMORY: Not found\n"));
    return RETURN_NOT_FOUND;
  }
  ...
  )
{
  ...
  if (CbFbRec == NULL) {
    DEBUG ((DEBUG_INFO, "No coreboot video frame buffer information found\n"));
    return RETURN_NOT_FOUND;
  }
